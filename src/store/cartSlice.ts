...
const cartSlice = createSlice({
  ...
  reducers: { // MỞ CỬA REDUCERS
    ...
    addToCart: (state, action: PayloadAction<CartItem>) => {
      ...
      const itemInCart = state.find(item => item.id === itemClick.id && item.size === itemClick.size);
      if (itemInCart) {
    ...
    changeQuantity: (state, action: PayloadAction<{ id: number; size: string; amount: number }>) => {
